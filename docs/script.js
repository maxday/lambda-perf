const load = async () => {
	const request = await fetch('https://raw.githubusercontent.com/maxday/lambda-perf/main/data/last.json?0.28471440814906934')
...
const drawLang = async (idx, data) => {
	...
	const sanitizedRuntimeName = runtime.runtime
		.replace('lambda-perf-','')
		.replaceAll('_', ' ')
		.replaceAll('native', ' ')
