...
const buildJsonFromData = (data) => {
    ...
    for(const runtime of runtimes) {
        ...
        const initDurations = filteredData.map(e => parseFloat(e.initDuration.S, 10));
        const memorySize = parseInt(filteredData[0].memorySize.S, 10);
        const averageMemoryUsed = formatMaxThreeDigits(computeMean(filteredData.map(e => parseFloat(e.maxMemoryUsed.S, 10))));
        const averageDuration = formatMaxThreeDigits(computeMean(filteredData.map(e => parseFloat(e.duration.S, 10))));
        const averageColdStartDuration = formatMaxThreeDigits(computeMean(initDurations));
        runtimeData.push({ runtime, initDurations, memorySize, averageMemoryUsed, averageDuration, averageColdStartDuration});
    }
